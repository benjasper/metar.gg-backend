...
func (Metar) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Float("dewpoint").Optional().Nillable().Comment("The dewpoint in Celsius.").Annotations(entgql.Skip(entgql.SkipType)),
		field.Int("wind_speed").Optional().Nillable().Comment("The wind speed in knots, or 0 if calm.").Annotations(entgql.Skip(entgql.SkipType)),
		field.Int("wind_gust").Optional().Nillable().Comment("The wind gust in knots.").Annotations(entgql.Skip(entgql.SkipType)),
		field.Int("wind_direction").Optional().Nillable().Comment("The wind direction in degrees, or 0 if calm."),
		field.Bool("wind_direction_variable").Comment("Whether the wind direction is variable (VRB)"),
		field.Float("visibility").Optional().Nillable().Comment("The visibility in statute miles.").Annotations(entgql.Skip(entgql.SkipType)),
		field.Bool("visibility_is_more_than").Comment("Whether the visibility is more than it's assigned value (+)"),
		field.Float("altimeter").Optional().Nillable().Comment("The altimeter setting in inches of mercury.").Annotations(entgql.Skip(entgql.SkipType)),
