...
func FlightCategoryValidator(fc FlightCategory) error {
	...
	case FlightCategoryVFR, FlightCategoryMVFR, FlightCategoryIFR, FlightCategoryLIFR:
		return nil
	default:
		return fmt.Errorf("taf: invalid enum value for flight_category field: %q", fc)
	}
}
...
func MetarTypeValidator(mt MetarType) error {
	switch mt {
	case MetarTypeTAF, MetarTypeSPECI:
		return nil
	default:
		return fmt.Errorf("taf: invalid enum value for metar_type field: %q", mt)
	}
}
